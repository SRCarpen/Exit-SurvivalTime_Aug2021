...
load(file='Results_Step4ET_Tuesday1315.Rdata')
fname = c('Survival_Left_Tuesday1315.Rdata')

windows()
par(mfrow=c(1,1),mar=c(4,4.5,2,2)+0.1,cex.axis=1.6,cex.lab=1.6)
xrange=range(c(D1s$x,sigmas$x))
yrange = range(c(D1s$y,sigmas$y))
...
# Make grid
N = 1000
xs = seq(from=xeq[1]-1,to=(xeq[2]-0.01),length.out=N)
dx = diff(range(xs))/N
...
plot(xs,drif,type='l',main='test functions for D1 and D2')
abline(h=0,lty=2)
plot(xs,dif,type='l')
#plot(xs,dD1,type='l')
#plot(xs,dD2,type='l')
...
text(x=-0.5,y=80,paste('Weighted mean = ',round(Shalf.wts,2)),cex=1.5)
grid()
abline(v=xeq,lty=c(2,2,2),lwd=c(2,2,2),col=c('blue','red','blue'))
