...
#
#
load('DDbintau_Peter1315.Rdata')
Fname.out = c('Results_Step4ET_Peter1315.Rdata')
piglab=c('Chlorophyll')
#load('Paul1315_DDbin9tau.Rdata')
#Fname.out = c('Results_Bintauweights_plots_Paul1315.Rdata')
#piglab ='Phycocyanin'
#load('DDbintau_Tuesday1315.Rdata')
#Fname.out = c('Results_Step4ET_Tuesday1315.Rdata')
...
legend('topright',legend=c('Drift','Diffusion as s.d.'),lty=c(1,2),lwd=c(2,2),
...
x = seq(-7,xeq[2],length.out=30)  # wide range
...
# Right basin 
  ...
  x = seq(xeq[2],7,length.out=30)  # wide interval
  # solve with bvpcol or bvptwp
  trycol <- bvptwp(yini = yini, x = x, func = feval2, yend = yend, parm=plist)
...
save(Tstep,Xvar,D1x,D1y,D1s,D2s,sigmas,xeq,xvec,drift,diff,negPF,
