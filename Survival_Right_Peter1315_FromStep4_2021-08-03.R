...
fname = c('Survival_Right_Peter1315.Rdata')
...
# Make grid
N = 1000
xs = seq(from=(xeq[2]+0.01),to=(xeq[3]+0.5),length.out=N)
dx = diff(range(xs))/N
...
#seq(1,length(times),1000) # time step values for subset
...
plot(zhalf$y,zhalf$x,type='l',lwd=2,col='blue',xlab='phycocyanin',ylab='Half-Life, hours',
     main='Bloom Half Life')
text(x=2,y=75,paste('Weighted mean = ',round(Shalf.wts,2)),cex=1.5)
grid()
abline(v=xeq,lty=c(2,2,2),lwd=c(2,2,2),col=c('blue','red','blue'))
