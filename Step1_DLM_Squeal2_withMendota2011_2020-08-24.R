...
#save(Paul_BGA,Pete_BGA,Tues_BGA,file='Squeal2_EachLake_AllYears.Rdata')
load(file='Squeal2_EachLake_AllYears.Rdata')

# show variates in data file
print(Tues_BGA[1,])

# Example: choose lake L, R, T, variate, and title-------------------------------------
uselake = Tues_BGA # lake choice
lakesub = subset(uselake,select=c('Year','DoY','BGA_logged_HYLB'))
title = c('log phyco, Tuesday, 2013-2015')
Fname = c('Tuesday1315_DLMresult.Rdata')
